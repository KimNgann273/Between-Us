// Camera: where the viewer is looking in the world, and how close.
// The world has fixed units (see WORLD in growth.js), so the layout is the
// same on every screen; only the zoom that fits it changes.
...
const Camera = {
  ...
  // Scroll zoom. With an anchor (a screen point), the world point under the
  // anchor stays where it is, so the zoom heads toward the cursor.
  ...
  // Drag pan, in screen pixels. Moves the eased position too, so the world
  // tracks the cursor exactly instead of lagging behind it.
  ...
  // 0 when looking closely at a spore, 1 when the whole field is in view.
  zoomOutProgress() {
