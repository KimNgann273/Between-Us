// Texture: the grain, the soil and the vignette that sit under and over the
// world. Without them the picture is clean vector shapes on flat black, which
// reads as a diagram; with them it reads as something alive under the ground.
//
// Everything here is generated once, in Texture.build(), and only drawn each
// frame, so the per-frame cost is a handful of image() calls.
//
// Three layers, outermost last:
//   soil     mottled noise behind the world, inside the camera (world units)
//   motes    specks of dust drifting in that soil, also in world units
//   vignette + grain, over the finished picture, in screen pixels
...
// Any of these can be set to 0 to turn that layer off.
...
const Texture = {
  ...
  // Called once in setup(), and again on resize (only the vignette depends on
  // the canvas size; the rest is in world units or tiled).
  build() {
  ...
  // A few tiles of white speckle. The alpha is squared so most pixels are
  // nearly clear and only a few are bright: that is what grain looks like,
  // where an even scatter looks like television static.
  buildGrain() {
  ...
  // The ground itself: two scales of Perlin noise, broad patches with a finer
  // mottle inside them, drawn small and scaled up so it stays soft.
  buildSoil() {
  ...
  // Math.random, not p5's random(), so building this never disturbs the seeded
  // stream that lays out the world.
  buildMotes() {
  ...
  // Call inside Camera.apply(), before anything living is drawn.
  ground(time) {
    if (this.soil) {
  ...
  // Call after the camera has been popped, over the finished picture.
  overlay(time) {
    if (this.vignette) image(this.vignette, 0, 0, width, height);
    if (!this.grainTiles.length) return;

    // One tile per grain step, shifted by a different amount each step, so the
    // repeat never settles into a visible pattern.
...
// Fades the soil out near its own edges, so when the whole world is on screen
// there is no rectangle of texture sitting on the background.
