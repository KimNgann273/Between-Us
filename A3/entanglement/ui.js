// Page chrome: phase tabs, prev/next arrows, the Narrator's caption, and the
// Home and Outro screens. Knows nothing about the world itself.

const CAPTION_FADE_MS = 350;   // matches .caption transition in styles.css
const OUTRO_FADE_MS = 2400;    // matches .outro transition
const CLOSING_FADE_MS = 1000;  // matches .closing-line transition
const CLOSING_GAP_MS = 350;    // darkness held between two closing lines
const CLOSING_LAST_MS = 1200;  // the last line gets longer, before Home arrives

// A line arrives a word at a time. Each word waits on the one before it: a
// base beat, a little more for a long word, and a breath at punctuation.
const WORD_MS = 90;
const WORD_MS_PER_CHAR = 15;
const WORD_MS_CAP = 120;
const BREATH_MS = 130;         // after , ; : - a comma's worth of pause
const FULL_STOP_MS = 250;      // after . ? ! ...

// The gap before the word that follows this one.
...
// A line in (brackets) is not the Narrator speaking — it is the piece telling
// the viewer what to do. It is set differently (.is-hint in styles.css) and it
// stays up longer.
function isHint(line) {
  return /^\(.*\)$/.test(line.trim());
}

// How long a finished line rests before the next one. Shorter than it looks:
// the reveal already gave the viewer most of their reading time.
function readingTime(line) {
  const rest = 1000 + line.length * 20;
  // A hint has to outlast the reveal long enough for them to act on it.
  return isHint(line) ? rest + 1600 : rest;
}

// The Outro's lines are not word-revealed — each one arrives whole, rests, and
// goes — so they keep their own beat, slower than the Narrator's `readingTime`.
// The rate is per character, and it has to be a rate a long sentence survives:
// the old 40ms/char was set when the Outro was one short line, and gave the
// longest of these nearly six seconds on screen.
...
// The Narrator's caption. say() queues lines and plays them one at a time,
// revealing each word by word; clear() cuts whatever is playing (used when a
// new moment happens); skip() fills the rest of the current line in at once.
const Caption = {
  el: null,
  queue: [],
  token: 0,
  playing: false,
  revealing: false,  // true only while words are still arriving, which is the
  words: [],         // only time skip() has anything to do
  current: null,
  wordTimer: null,
  holdTimer: null,
  fadeTimer: null,
  ...
  // A click anywhere finishes the line that is still arriving. It never skips
  // to the next line: clicks are also how the viewer plays with the world, and
  // narration should not race ahead every time they touch a spore.
  skip() {
  ...
  _next(token) {
    ...
    if (!item) {
      this.playing = false;
      // Nothing left to say: let the world come back up. Between two lines the
      // class stays on, so the world does not breathe in and out per line.
      document.body.classList.remove('is-narrating');
      return;
    }
    this.playing = true;
  ...
  // Rebuilds the line as one span per word, all invisible, then lets them in
  // one by one. The spaces stay real text nodes so the line still wraps.
  _reveal(token, item) {
    this.current = item;
    this.el.textContent = '';
    this.el.classList.toggle('is-hint', isHint(item.line));
    // The world steps back for as long as the Narrator has something to say.
  ...
  // The line is fully up: let it sit, then move on.
  _settle(token, item) {
    this.revealing = false;
...
const UI = {
  ...
  // actions: { begin, home, goTo(i), prev, next }
  init(actions) {
    ...
    // A click anywhere fills in the rest of the line, for viewers who read
    // faster than the Narrator speaks. The listener neither stops the event
    // nor cancels it, so the same click still reaches the canvas.
  ...
  // Fade to black, then play the closing lines one at a time — each fading in
  // whole, resting, and fading out before the next — and let Home fade in under
  // the last one, which stays up.
  playOutro(lines) {
    ...
    const step = (i) => {
      ...
      setTimeout(() => {
        if (token !== this.outroToken) return;
        if (last) {
          // The ending stays on screen; the way back appears beneath it.
