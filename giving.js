// Phase 3 of the journey: Giving.
// Revived, the First Spore reaches back. Its hyphae find the struggling spores
// around it; each one it joins glows, grows a little, and reaches on for the
// next. Pull on any of them and the whole network flexes; nothing can be
// pulled away. Click one and a pulse travels out through everything it's tied to.
//
// Built on what Receiving introduced (fusion, junctions and pulses, all in
// receiving.js). Loop and input: sketch.js. Drawing: render.js.
...
const GIVING = {
  ...
  flexSpring: 9,
  holdSpring: 14,
  pullMax: 150,             // however hard you pull, a spore moves at most this far from home
  pullNotice: 40            // a pull this far makes the Narrator remark on it
};
...
const Giving = {
  ...
  update(dt) {
    ...
    // The board asks the viewer to zoom out. If they don't, the camera does it, once.
  ...
  // Pressing on a joined spore grabs it (sketch.js asks before it starts a pan).
  grab(point) {
    const s = givingState();
    let best = null;
...
// Giving's progress belongs to the world, so a fresh world starts it over.
function givingState() {
  if (world.giving) return world.giving;
  const joined = connectedTo(world.first);
  const s = {
    joined,                 // spores that are part of the network
    quota: new Map(),       // how many more reaching hyphae each may send
    readyAt: new Map(),     // when each may start sending them
    targeted: new Set(),    // spores something is already reaching for
    cords: [],              // Cords on their way; each holds its own strands
...
function spread(s) {
  // Hand on: joined spores that are ready reach for the nearest spores still alone.
  for (const spore of s.joined) {
    ...
    if (left > 0) s.readyAt.set(spore, s.time + 1); // nothing in reach yet; look again shortly
  }

  // Arrive: the first strand to reach the spore joins it. The rest keep growing
  // so the Cord closes up around the junction instead of stopping in mid air,
  // and each stops as it arrives.
  s.cords = s.cords.filter((cord) => {
    ...
    // A strand is fed by the network, so nothing stops it on its own: a stray
    // one would circle its spore forever. Once the Cord has arrived, whatever
    // is still out has a few seconds to come in, and then stops where it is.
    ...
    if (!cord.joined) {
      // Lost its way: free the spore for someone else, and give the sender another try.
...
// A Cord: two or three hyphae leaving for the same spore together. Each wanders
// as freely as any other hypha and only turns firmly toward the spore once it
// is close (lureRamp), so the strands drift apart on the way out and converge
// at the far end. None is limited by reach or budget, and all pass through
// other hyphae, leaving junctions, on the way.
...
// The held spore springs toward the cursor, but only so far. Every other joined
// spore is drawn the same way, less for each junction between it and the one
// being pulled, so the pull ripples through the whole network. On release
// every spore springs straight back home.
function flex(s, dt) {
  const pull = new Map(); // spore -> how far it should be displaced right now
  if (s.held) {
...
// A wave of light from one spore out through every junction it can reach,
// each spore passing it on as the pulse arrives.
...
// ---- flexing, for drawing ----------------------------------------------------------
// When a spore is pulled off home, the points of its hyphae move with it,
// fully at the spore and fading with distance along the hypha.
